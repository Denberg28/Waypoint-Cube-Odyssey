...
GOVERNANCE_PATH = ROOT / "runtime/development_governance.json"
    ...
    "properties": {
        ...
        "expansion": {
            ...
            "properties": {
                ...
                "landmark": {"type": "string"}, "hazard": {"type": "string"},
                "collectible": {"type": "string"}, "rationale": {"type": "string"}
            },
...
def exploration_locked() -> bool:
    governance = load(GOVERNANCE_PATH, {})
    locked = set(str(x) for x in governance.get("locked_categories", [])) if isinstance(governance, dict) else set()
    return "exploration" in locked
...
def validate_candidate(raw: dict, world_map: dict) -> dict | None:
    ...
    if not re.fullmatch(r"[a-z][a-z0-9_]{2,23}", node_id): return None
    existing = {str(n.get("id", "")) for n in world_map.get("nodes", []) if isinstance(n, dict)}
    if node_id in existing: return None
    links = []
    for value in raw.get("connect_to", []):
        value = str(value).strip()
        if value in existing and value not in links: links.append(value)
    ...
    if planned_count >= limit: return None
    x = max(0.08, min(0.92, float(raw.get("x", 0.5)))); y = max(0.08, min(0.92, float(raw.get("y", 0.5))))
    for n in world_map.get("nodes", []):
        if not isinstance(n, dict): continue
        dx = x - float(n.get("x", 0.5)); dy = y - float(n.get("y", 0.5))
        if dx * dx + dy * dy < 0.018: return None
    return {"id": node_id, "name": str(raw.get("name", "New Region")).strip()[:48], "kind": "region", "status": "planned", "playable": False,
            "difficulty": max(1, min(4, int(raw.get("difficulty", 2)))), "theme": str(raw.get("theme", "unknown frontier")).strip()[:80],
            "x": round(x, 3), "y": round(y, 3), "landmark": str(raw.get("landmark", "")).strip()[:100],
            "hazard": str(raw.get("hazard", "")).strip()[:100], "collectible": str(raw.get("collectible", "")).strip()[:100],
            "rationale": str(raw.get("rationale", "")).strip()[:320], "connect_to": links[:3]}


def apply_candidate(world_map: dict, candidate: dict) -> dict:
    result = json.loads(json.dumps(world_map)); result.setdefault("nodes", []).append(candidate); result.setdefault("edges", [])
    ...
    for target in candidate.get("connect_to", []):
        edge = (target, candidate["id"]); rev = (candidate["id"], target)
        if edge not in existing_edges and rev not in existing_edges: result["edges"].append({"from": target, "to": candidate["id"]})
...
def write_report(decision: dict, candidate: dict | None, reason: str) -> str:
    lines = ["# Waypoint World Architect — Latest Council Resolution", "", f"Decision: **{decision.get('decision', 'hold').upper()}**", "", str(decision.get("summary", reason)).strip() or reason, "", "## Council basis"]
    basis = decision.get("council_basis", []) if isinstance(decision, dict) else []
    for item in basis[:8]: lines.append(f"- {str(item)[:240]}")
    if not basis: lines.append("- No strong map-expansion signal was present.")
    lines += ["", "## Map action"]
    if candidate:
        lines += [f"- Added planned region **{candidate['name']}** (`{candidate['id']}`)", f"- Difficulty concept: {candidate['difficulty']}", f"- Theme: {candidate['theme']}", f"- Connections: {', '.join(candidate['connect_to'])}", f"- Landmark: {candidate['landmark']}", f"- Hazard: {candidate['hazard']}", f"- Collectible: {candidate['collectible']}", "- Status remains **planned / non-playable** until a later core-route promotion step validates gameplay support."]
    else: lines.append(f"- No map node added. {reason}")
...
def main() -> None:
    ...
    if exploration_locked():
        decision = {"decision": "hold", "summary": "Exploration milestone is optimization-locked; world expansion is frozen unless governance is explicitly reopened.", "council_basis": [], "expansion": {}}
        reason = "Milestone governor froze ordinary map expansion to prevent over-optimization."
        resolution = {"decision": "hold", "summary": decision["summary"], "map_node_added": None, "map_revision": int(world_map.get("revision", 0)), "specialist": "World Architect & Cartographer", "governance_hold": True}
        ...
        REPORT_PATH.write_text(write_report(decision, None, reason), encoding="utf-8")
    ...
    if support and decision.get("decision") == "expand":
        ...
        if candidate:
            world_map = apply_candidate(world_map, candidate); MAP_PATH.parent.mkdir(exist_ok=True); MAP_PATH.write_text(json.dumps(world_map, indent=2, ensure_ascii=False), encoding="utf-8"); reason = "Validated one bounded planned-region expansion."
        else:
            decision["decision"] = "hold"; reason = "Expansion proposal failed map guardrails or the planned-region cap."
    elif decision.get("decision") == "expand" and not support:
        decision["decision"] = "hold"; reason = "The six-agent council did not contain enough exploration/navigation evidence."

    resolution = {"decision": decision.get("decision", "hold"), "summary": str(decision.get("summary", reason))[:800], "map_node_added": candidate.get("id") if candidate else None, "map_revision": int(world_map.get("revision", 0)), "specialist": "World Architect & Cartographer"}
    council["world_architect"] = resolution
    COUNCIL_PATH.write_text(json.dumps(council, indent=2, ensure_ascii=False), encoding="utf-8")
    REPORT_PATH.parent.mkdir(exist_ok=True); REPORT_PATH.write_text(write_report(decision, candidate, reason), encoding="utf-8")
    print(json.dumps({"ok": True, **resolution}, ensure_ascii=False))
